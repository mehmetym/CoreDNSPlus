  ...
  /*  public static void installSecurityProvider(Context context) {
    ...
    public static void initEnvironment(Context context) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 19) {
            ...
            if (dir != null)
                WORKDIR = dir.getAbsolutePath() + "/CoreDNS+";

            String backwardcompWorkdir = "/storage/emulated/0/Android/data/dnsfilter.android/files/CoreDNS+";
            try {
            ...
            WORKDIR = Environment.getExternalStorageDirectory().getAbsolutePath() + "/CoreDNS+";
    }
    ...
    private void waitForStorage() {
        ...
        boolean pathExists = new File("/storage/emulated/0/Android/data/dnsfilter.android/files/CoreDNS+").exists();
    ...
    public void wakeLock(){
        ...
        wifiLock.acquire();
        PowerManager.WakeLock wakeLock = ((PowerManager) ctx.getSystemService(Context.POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "CoreDNS+:wakelock");
        wakeLock.acquire();
    ...
    public void migrateConfig() throws IOException {
        ...
        if (!F_WORKDIR.exists() && storagePermission) {
            File OLDPATH = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/CoreDNS+");
