/*
 ...
 Find the latest version at http://github.com/mehmetym/CoreDNSPlus/corednsplus
 Contact:i.z@gmx.net 
 */
...
public class DNSProxyActivity extends Activity
	...
	protected static CheckBox darkModeCheck;
	...
	private class MyUIThreadLogger implements Runnable {
		...
		public synchronized void run() {
			...
			setTitle("CoreDNS+ V" + version + " (Connections:" + connCnt + ")");
	...
	public void onCreate(Bundle savedInstanceState) {
		try {
			...
			if (prefs.contains("override_dark_mode")) {
				boolean isDark = prefs.getBoolean("override_dark_mode", false);
				...
				if (android.os.Build.VERSION.SDK_INT >= 31) {
					...
					if (uiManager != null) {
						uiManager.setApplicationNightMode(isDark ? android.app.UiModeManager.MODE_NIGHT_YES : android.app.UiModeManager.MODE_NIGHT_NO);
					}
				}
				
				android.content.res.Configuration config = new android.content.res.Configuration(getResources().getConfiguration());
				int targetMode = isDark ? android.content.res.Configuration.UI_MODE_NIGHT_YES : android.content.res.Configuration.UI_MODE_NIGHT_NO;
				if ((config.uiMode & android.content.res.Configuration.UI_MODE_NIGHT_MASK) != targetMode) {
					config.uiMode = (config.uiMode & ~android.content.res.Configuration.UI_MODE_NIGHT_MASK) | targetMode;
			...
			if (logTxt != null)
				...
				logOutView.setText(fromHtml("<strong><em>****This is CoreDNS+ V" + DNSFilterManager.VERSION + "****</em></strong><br><br>"));
			...
			checked = darkModeCheck != null && darkModeCheck.isChecked();
			darkModeCheck = (CheckBox) findViewById(R.id.darkModeCheck);
			android.content.SharedPreferences darkModePrefs = getSharedPreferences("dnsfilter_prefs", android.content.Context.MODE_PRIVATE);
			boolean isDark = darkModePrefs.getBoolean("override_dark_mode", false);
			darkModeCheck.setChecked(isDark);
			darkModeCheck.setOnClickListener(this);
	...
	public void onResume() {
		try {
			super.onResume();
			handleAdvancedConfig(advancedConfigCheck);
			checkPasscode();
		} catch (Exception e){
			e.printStackTrace();
	...
	protected void doRestoreDefaults() {
		...
		try {
			...
			Logger.getLogger().logLine("Can not load /CoreDNS+/additionalHosts.txt!\n" + eio.toString());
		}
	}
	...
	protected void loadManuallyEditConf()  {
		try {
			...
			Logger.getLogger().logLine("Can not load /CoreDNS+/dnsfilter.conf!\n" + eio.toString());
		}
	}
	...
	public void onClick(View destination) {
		...
		if (destination == addFilterBtn) {
			...
			openBrowser("https://www.zenz-home.com/corednsplus/help/help.php");
			return;
		...
		if (destination == darkModeCheck) {
			boolean newDark = darkModeCheck.isChecked();
			android.content.SharedPreferences themePrefs = getSharedPreferences("dnsfilter_prefs", android.content.Context.MODE_PRIVATE);
			themePrefs.edit().putBoolean("override_dark_mode", newDark).apply();
			
			if (android.os.Build.VERSION.SDK_INT >= 31) {
				android.app.UiModeManager uiManager = (android.app.UiModeManager) getSystemService(android.content.Context.UI_MODE_SERVICE);
				if (uiManager != null) {
					uiManager.setApplicationNightMode(newDark ? android.app.UiModeManager.MODE_NIGHT_YES : android.app.UiModeManager.MODE_NIGHT_NO);
				}
				return;
			}
			
			android.content.res.Configuration config = new android.content.res.Configuration(getResources().getConfiguration());
			int targetMode = newDark ? android.content.res.Configuration.UI_MODE_NIGHT_YES : android.content.res.Configuration.UI_MODE_NIGHT_NO;
			config.uiMode = (config.uiMode & ~android.content.res.Configuration.UI_MODE_NIGHT_MASK) | targetMode;
			getResources().updateConfiguration(config, getResources().getDisplayMetrics());
